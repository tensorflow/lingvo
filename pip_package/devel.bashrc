...
function refresh_prompt() {
  _TF_VERSION=$(pip3 show tensorflow | grep Version | cut -f 2 -d' ')
  _PY_VERSION=$(python --version | cut -f 2 -d' ')
  export PS1="\[\e[31m\]lingvo-pip (TF ${_TF_VERSION}; PY ${_PY_VERSION})\[\e[m\] \[\e[33m\]\w\[\e[m\] > "
}
refresh_prompt
