...
def CellFnFropOpReplacementWrapper():
  ...
  saved_get_step_seed = py_utils.GetIncStepSeed
  ...
  py_utils.GetIncStepSeed = GetStepSeed

  yield

  tf.assert_equal = saved_assert_equal
  py_utils.GetIncStepSeed = saved_get_step_seed
  ...
  def FProp(self, theta, *args):
    ...
    with tf.device(devices[0]):
      ...
      inputs[_MICRO_BATCH_STATE_NAME] = tf.stack(
          [tf.cast(t, dtype=state_dtype) for t in range(p.num_micro_batches)])
