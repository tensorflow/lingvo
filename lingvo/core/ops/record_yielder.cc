...
void RecordYielder::MainLoop() {
  while (true) {
    ...
    const int num = filenames.size();
    if (0 <= opts_.file_shuffle_shift_ratio &&
        opts_.file_shuffle_shift_ratio < 1) {
      const int shift = int(opts_.file_shuffle_shift_ratio * num);
      LOG(INFO) << "File shift = " << shift << " among " << num << " files";
      std::vector<string> left(shift);
      for (int i = 0; i < shift; ++i) {
        std::swap(left[i], filenames[i]);
      }
      for (int i = shift; i < num; ++i) {
        std::swap(filenames[i - shift], filenames[i]);
      }
      for (int i = 0; i < shift; ++i) {
        std::swap(filenames[num - shift + i], left[i]);
      }
    }
