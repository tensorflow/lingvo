...
class ProjectionLayer(quant_utils.QuantizableLayer):
  ...
  def _ApplyProjectionKernel(self,
    ...
    if p.use_blocked_matmul:
      ...
      out = self.QEinsum('bn,nmk->bmk', x, w)
      ...
      inputs, w = self.ToAqtInputs('w', act=inputs, weight=w, w_feature_axis=-1)
      if p.use_block_diagonal_matmul:
        out = py_utils.BlockDiagonalProjectLastDim(
            inputs,
            w,
            p.input_dim,
        ...
        qlayer = self if p.qdomain is not None else None
    ...
    return gshard_utils.MeshSplit(
        out, p.device_mesh, p.activation_split_dims_mapping
    )
...
class MultitaskProjectionEinsumLayer(quant_utils.QuantizableLayer):
  ...
  def Params(cls):
    ...
    p.Define(
        'einsum_order',
        'multiply_and_select',
        'The order of performing selection and multiplication. If the '
  ...
  def FProp(self, theta, inputs, tasks, paddings=None):
    ...
    with tf.name_scope(p.name):
      ...
      out = self._ApplyProjectionKernel(theta, inputs, tasks)
      out = self._ApplyActivationFunction(out)
      return out
  ...
  def _ApplyProjectionKernel(self, theta, inputs, tasks):
    """Applies projection.
